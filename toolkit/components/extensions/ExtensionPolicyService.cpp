...
ExtensionPolicyService::IsExtensionProcess() const
{
  if (sRemoteExtensions && XRE_IsContentProcess()) {
    ...
    dom::ContentChild::GetSingleton()->GetProcessName(processName);
    return processName.EqualsLiteral("webextension");
  }
...
{
  if (aURL.Scheme()->Equals(NS_LITERAL_STRING("moz-extension"))) {
...
{
  if (WebExtensionPolicy* policy = GetByID(aAddonId)) {
    *aResult = policy->CanAccessURI(aURI);
    return NS_OK;
  }
  return NS_ERROR_INVALID_ARG;
}
