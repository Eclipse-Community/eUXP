...
MatchPattern::Init(JSContext* aCx, const nsAString& aPattern, bool aIgnorePath, ErrorResult& aRv)
{
  ...
  if (host.IsEmpty() && !scheme->Equals(NS_LITERAL_STRING("file"))) {
    aRv.Throw(NS_ERROR_INVALID_ARG);
    return;
  }
...
{
  if (!mSchemes->Contains(NS_LITERAL_CSTRING("https")) &&
      (aCookie.IsSecure() || !mSchemes->Contains(NS_LITERAL_CSTRING("http")))) {
    return false;
  }
